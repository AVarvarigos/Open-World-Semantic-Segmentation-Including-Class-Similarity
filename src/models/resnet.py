...
class NonBottleneck1D(nn.Module):
    """
    ERFNet-Block
    Paper: http://www.robesafe.es/personal/eduardo.romera/pdfs/Romera17tits.pdf
    Code: https://github.com/Eromera/erfnet_pytorch/blob/master/train/erfnet.py
    """
    expansion = 1
    ...
    def __init__(
        ...
        super().__init__()
        dropprob = 0
        self.conv3x1_1 = nn.Conv2d(inplanes, planes, (3, 1), stride=(stride, 1), padding=(1, 0), bias=True)
        self.conv1x3_1 = nn.Conv2d(planes, planes, (1, 3), stride=(1, stride), padding=(0, 1), bias=True)
        self.bn1 = nn.BatchNorm2d(planes, eps=1e-03)
        self.act = activation
        self.conv3x1_2 = nn.Conv2d(planes, planes, (3, 1), padding=(1 * dilation, 0), bias=True, dilation=(dilation, 1))
        self.conv1x3_2 = nn.Conv2d(planes, planes, (1, 3), padding=(0, 1 * dilation), bias=True, dilation=(1, dilation))
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=dilation[1], replace_stride_with_dilation=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=dilation[2], replace_stride_with_dilation=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=dilation[3], replace_stride_with_dilation=replace_stride_with_dilation[2])
...
def ResNet18(pretrained_on_imagenet=False, pretrained_dir="./trained_models/imagenet", **kwargs):
    ...
    if kwargs["block"] != BasicBlock and pretrained_on_imagenet:
        ...
        if "input_channels" in kwargs and kwargs["input_channels"] == 1:
            ...
            weights["conv1.weight"] = torch.sum(weights["conv1.weight"], axis=1, keepdim=True)
        weights.pop("fc.weight")
        weights.pop("fc.bias")
        model.load_state_dict(weights, strict=True)
        print("Loaded ResNet-18 pretrained on ImageNet")
    return model


def ResNet34(pretrained_on_imagenet=False, pretrained_dir="./trained_models/imagenet", **kwargs):
    if "block" not in kwargs:
        kwargs["block"] = BasicBlock
    else:
    ...
    if kwargs["block"] != BasicBlock and pretrained_on_imagenet:
        ...
        weights.pop("fc.bias")
        model.load_state_dict(weights, strict=True)
        print("Loaded ResNet-34 pretrained on ImageNet")
    return model
...
def ResNet50(pretrained_on_imagenet=False, **kwargs):
    ...
    if pretrained_on_imagenet:
        ...
        weights.pop("fc.bias")
        model.load_state_dict(weights, strict=True)
        print("Loaded ResNet-50 pretrained on ImageNet")
    return model
...
def load_pretrained_with_different_encoder_block(
    ...
    if not os.path.exists(ckpt_path):
        ...
        print(f"Choosing checkpoint {ckpt_path} with top-1 acc {acc_top1}")
    ...
    for key in checkpoint["state_dict"]:
        if "encoder" in key:
            checkpoint["state_dict2"][key.split("encoder.")[-1]] = checkpoint["state_dict"][key]
    ...
    print(f"Loaded {resnet_name} with encoder block {encoder_block} pretrained on ImageNet")
    print(ckpt_path)
    return model
...
if __name__ == "__main__":
    ...
    model = ResNet50( # Choose from ResNet18, ResNet34, ResNet50
        #block="BasicBlock", # Choose from BasicBlock, NonBottleneck1D for ResNet-18 and ResNet-34 only
