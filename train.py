...
def train_main():
    ...
    loss_function_valid = utils.CrossEntropyLoss2dForValidData(weight=class_weighting, weighted_pixel_sum=pixel_sum_valid_data_weighted, device=device)
...
def validate(
    ...
    compute_iou = IoU(task="multiclass", num_classes=classes, average="none", ignore_index=255).to(device)
...
def test_ow(
    ...
    compute_iou = IoU(task="multiclass", num_classes=2, average="none", ignore_index=255).to(device)
    ...
    for i, sample in enumerate(tqdm(test_loader, desc="Testing")):
        ...
        with torch.no_grad():
            ...
            clusters = ac(n_clusters=None, affinity="euclidean", distance_threshold=0.5).fit(preds.cpu().numpy().T)
